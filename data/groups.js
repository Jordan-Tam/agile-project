...
const exportedMethods = {
   ...
   async getGroupByID(id) {
        ...
        if (group.groupMembers && group.groupMembers.length > 0) {
            ...
            const matched = allUsers.find(u => u._id.toString() === memberId.toString());
            if (matched) {
                return {
                ...
                return { firstName: "Unknown", lastName: "", _id: memberId.toString() };
            }
            });
        } else {
            group.groupMembers = [];
    ...
    async getAllGroups() {
        ...
        return allGroups.map(g => ({
            ...g,
            _id: g._id.toString(),
            groupMembers: g.groupMembers ? g.groupMembers.map(m => m.toString()) : []
        }));
    },
    ...
    async addMember(groupId, first_name, last_name, user_id) {
        ...
        if (updatedGroup.groupMembers) {
            updatedGroup.groupMembers = updatedGroup.groupMembers.map(m => m.toString());
        } else {
