...
async function testBalanceCalculation() {
	...
	try {
		...
		if (allUsers.length < 3) {
			console.log(
				"Not enough users in database. Please run mainTests.js first."
			);
			return;
		}
		...
		console.log(
			`  ${user2.firstName} owes ${user1.firstName}: $${
				balances[user2._id.toString()]?.[user1._id.toString()] || 0
			}`
		);
		console.log(
			`  ${user3.firstName} owes ${user1.firstName}: $${
				balances[user3._id.toString()]?.[user1._id.toString()] || 0
			}`
		);
